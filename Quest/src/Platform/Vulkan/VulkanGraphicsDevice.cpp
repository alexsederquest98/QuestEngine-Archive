...
namespace Quest
{
	...
	static VkVertexInputBindingDescription GetVertexInputBindingDescription()
	{
		...
		bindingDescription.stride = sizeof(qmath::Vertex);
	...
	static std::array<VkVertexInputAttributeDescription, 2> GetVertexInputAttributeDescriptions()
	{
		...
		attributeDescriptions[0].offset = offsetof(qmath::Vertex, pos);

		attributeDescriptions[1].binding = 0;
		attributeDescriptions[1].location = 1;
		attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
		attributeDescriptions[1].offset = offsetof(qmath::Vertex, color);

		return attributeDescriptions;
	}

	const std::vector<qmath::Vertex> vertices = {
	...
	void VulkanGraphicsDevice::CreateDescriptorSetLayout()
	{
		...
		if (vkCreateDescriptorSetLayout(m_Device, &layoutInfo, nullptr, &m_DescriptorSetLayout) != VK_SUCCESS)
			QE_CORE_FATAL("Failed to create vulkan descriptor set layout");
	}
	...
	void VulkanGraphicsDevice::CreateTextureImage()
	{
		...
		stbi_uc* pixels = stbi_load("../../Quest/assets/textures/statue.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void VulkanGraphicsDevice::CreateDescriptorSets()
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			descriptorWrite.dstBinding = 0; // binding index of the uniform buffer
	...
	void VulkanGraphicsDevice::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		...
		{
			// just to get rid of undefined warnings
			barrier.srcAccessMask = 0;
			barrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
	...
	VkImageView VulkanGraphicsDevice::CreateImageView(VkImage image, VkFormat format)
	{
